#include "zakiomni.hpp"
 ...
 //: Node("hardware_control_" + std::to_string(tx_device_id)),
       ...
       //rclcpp::Time current = this->get_clock()->now();//何故か知らないけど間にget_clock()挟まないとコンパイルエラーと化した<-なんか消してもビルド通った
        ...
        std::cout << "Waiting for receiving topics" << std::endl;//なにもトピックを受け取ってないときの状態が欲しかった
    ...
    // デバッグ用のログ出力
    if(rps_num_count != 1 && rps_num_count != 3) {//回転しているエンコーダの数が1か2のときは正常に走行できている可能性が高いからログを出す（1,3輪で動くようなものは実装していない）
    ...
    }else if(count_true > count_false){//回転しているエンコーダの数が3のときは空転している可能性が高いからログを出す
    ...
    }else{//そんなわけないと思うけど一個だけ回転してる（と思われる）ときはこの警告を出す
