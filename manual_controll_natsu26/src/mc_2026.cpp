<<<<<<< HEAD:manual_controll_natsu26/src/mc_2026.cpp
/*
naturobo機構制御
...
// 以下マイコンに合わせて設定
#define OUTPUT_DEVICE_ID 2// 出力マイコン（モーター制御）のID
#define INPUT_DEVICE_ID 1  // 入力マイコン（マイクロスイッチやエンコーダ）のID
#define TX16NUM 24         // 送信データ数
#define RX16NUM 17         // 受信データ数
...
private:
    void ps4_listener_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
    {
        ...
        // TRIANGLE:　「小鰻射出機構」（ブラシレスモーター使用？）
            ...
            static int injection_speed = 70; // 射出速度の初期値 
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::shutdown();
    return 0;
>>>>>>> 116ef1b (射出機構用に少しだけプログラム修正):natsurobo_early_access/src/mc_2026.cpp
}